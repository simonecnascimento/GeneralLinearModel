...
for i = 1:numel(contents)
    if contents(i).isdir 
        ...
        break;  % Exit the loop if a valid subfolder is found
        end
    end
end
...
% Check for the existence of each substack option
for i = 1:numel(substackOptions)
    currentOption = substackOptions{i};
    if contains(folderPath, currentOption)
        substackString = currentOption;
        break;  % Exit the loop if a valid option is found
    end
end

if isempty(substackString)
    error('No valid substack folder found.');
end

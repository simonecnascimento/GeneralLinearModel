...
%         cellsToMerge = array2table(dataMatrix, 'VariableNames', cellstr(arrayfun(@(x) sprintf('Column%d', x), 1:maxLength, 'UniformOutput', false)));
        ...
        % Iterate through cellsToMerge
            ...
            rowsToRemove = any(dataTable{:, 2:end} == valuesToRemove, 2);

            cellsToMerge = cell2mat(cellsToMergeCell);
            rowsToRemove = any(cellfun('isempty', cellsToMergeCell(:, 2:end)), 2);
            dataTable(valuesToRemove,:) = [];

...
for s = 1:numel(contents)
    if contents(s).isdir && contains(contents(s).name, 'Substack') 
        ...
        %Add feature(number of events)
        newRowName = 'Number of Events';
        % Getting the existing row names
        existingRowNames = resultsFinal.Properties.RowNames;
        % Adding the new row name to the existing row names
        newRowNames = [existingRowNames; newRowName];
        % Adding a new row to the table (e.g., with zeros)
        newRowData = zeros(1, width(resultsFinal));
        resultsFinal = [resultsFinal; array2table(newRowData, 'VariableNames', resultsFinal.Properties.VariableNames)];
        % Setting the updated row names to the table
        resultsFinal.Properties.RowNames = newRowNames;
        ...
        % Ask the user to enter multiple sets of cells to remove
        %cellsToDelete = input('Enter cells to delete (e.g., [1,2]): ');
        % Check if userInput is valid (you may want to add more validation)
%         if isempty(cellsToDelete)
%             disp('There are no cells to delete');        
%         else
%             if isnumeric(cellsToDelete) && ismatrix(cellsToDelete)                          
%             % Iterate through cellsToDelete
%                 for m = 1:size(cellsToDelete, 1)
%                     % Extract cell indices to delete
%                     cellIndices = cellsToDelete(m, :);
%             
%                     % Combine events of CFUs
%                     combinedCellEvents = cat(1, cellList_eventList(cellIndices).eventList);
%                 
...
%         end   
        ...
        for c = 1:size(cellList_eventList,1)
            ...
            resultsFinal.Properties.RowNames{end+1} = newRowName;
            resultsFinal = [resultsFinal; newRow];

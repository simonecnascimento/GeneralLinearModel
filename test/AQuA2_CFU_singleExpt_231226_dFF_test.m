...
for s = 1:numel(contents)
    if contents(s).isdir && contains(contents(s).name, 'Substack') 
        ...
        'D:\2photon\Simone\Simone_Macrophages\', mouse, '\', [date '_FOV' num2str(fov)], '\AQuA2\1Hz', [subfolderName, '_AQuA_res_cfu_edited']);
        ...
        % Iterate through cellsToMerge
            for m = 1:size(cellsToMerge, 1)
                ...
                combinedCellEvents = cat(1, dataTable(cellIndices).eventList);
                % Update the first CFU with the combined events
                dataTable(cellIndices(1)).eventList = combinedCellEvents;

                % Extract dFF values for the specified cell indices
                dFF_values_to_combine = cell2mat(dFF_list(cellIndices));
                combined_dFF = mean(dFF_values_to_combine);
                % Update the first CFU with the combined dFF
                dataTable(cellIndices(1)).dFF_list = combined_dFF;
            end
        ...
        legend();
        saveas(dFF_curve, (fullfile(outputDirFOV, strcat(fileTemp,'_dFF_curve'))));
               
        %Adjustments to Results table before saving
        resultsFinal{'Cell ID'} = str2double(resultsFinal{'Cell ID'});
        resultsFinal = table2array(resultsFinal);
        resultsFinal = resultsFinal(:, any(~isnan(table2array(resultsFinal)), 1)); %remove row in which cell has 0 events (e.g: event was remove but cell was not merged. ie. cell was an artifact)

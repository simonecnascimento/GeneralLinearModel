...
for s = 1:numel(contents)
    if contents(s).isdir && contains(contents(s).name, 'Substack') 
        ...
        cellsToMerge = input('Enter ID of cells to merge (e.g., {[1,2]; [3,4,5]}): ');
        
        %If cellsToMerge does not have same length
        ...
        % Iterate through cellsToMerge
            ...
            for i = 1:size(cellsToMerge, 1)
                for x = 1:size(cellsToMerge, 2)
                    if isnan(cellsToMerge(i, x))
                        cellsToMergeCell{i, x} = [];
                    else
                        cellsToMergeCell{i, x} = cellsToMerge(i, x);
                    end
                end
            end
        ...
        for c = 1:size(resultsTable,1)
            
            % From one cell, extract its ID and its list of events - do for all cells
            cellID = resultsTable(c).cellList;
            cellEvents = resultsTable(c).eventList;
            ...
            % Assuming c is the value you are looking for in row 13
            cLabel = ['cell ' num2str(c)];

            % Find the column index
            cIndex = [];
            for col = 1:numel(results(13, :))
                if contains(results{13, col}, cLabel, 'IgnoreCase', true)
                    cIndex = col;
                    break;
                end
            end

            cIndex = find(contains(results(13, :), cLabel, 'IgnoreCase', true));
            if ~isempty(cIndex)
                if ismember(c, perivascularCells)
                    results{13, cIndex} = 0;
                elseif ismember(c, adjacentCells)
                ...
                disp(['Column not found for ' cLabel]);
            end

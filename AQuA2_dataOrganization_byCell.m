...
sheet = 'MacrophageBaseline_craniotomy';
...
for s = 1:numel(contents)
    if contents(s).isdir && contains(contents(s).name, 'Substack') 
        ...
        save(fullfile(outputDirFOV, strcat(fileTemp,'_analysisByCell'))); % save metadata inside FOV folder       
        save(fullfile(outputDirAnimal, strcat(fileTemp,'_analysisByCell'))); % save metadata inside animal folder
